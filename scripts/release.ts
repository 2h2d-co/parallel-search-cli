...
const packageName = "parallel-search-cli";
...
const executablePackageFiles: string[] = ["bin/parallel-search.js"];
...
async function buildPackageFromIndex(releaseVersion: string): Promise<string> {
  const temporaryRoot = await mkdtemp(join(tmpdir(), "parallel-search-cli-release-"));
